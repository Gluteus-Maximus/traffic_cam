...
#TODO: add time window to config - delete logs prior to window (call in store_netdev)
  # allow unbounded?  default value in globals.  filesize?
#TODO: replace hardcode .py helpstrings with __file__ formats
#TODO: add setup.sh (update PATH in bashrc)


### GLOBAL VARIABLES ###
global globalProgramDir
globalProgramDir = os.path.dirname(os.path.realpath(__file__))
configFile = os.path.join(globalProgramDir, '.traffic_cam.conf')
configDefaults = {
    'interface': None,
    'frequency': 1,
...
def load_config(filepath=configFile):
  ...
  try:
    ...
    raise FileNotFoundError(
        "CONFIG LOAD ERROR: config file missing - './traffic_cam config -h'"
        ) from e
  except json.decoder.JSONDecodeError as e:
    raise json.decoder.JSONDecodeError(
        "CONFIG LOAD ERROR: bad config file - './traffic_cam config -h'",
        ) from e
  except PermissionError as e:
    raise PermissionError(
        "CONFIG LOAD ERROR: permission denied - './traffic_cam config -h'",
        ) from e
